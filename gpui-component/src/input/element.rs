...
impl Element for TextElement {
    ...
    fn paint(
        ...
        if window.is_window_active() {
            ...
            if let Some(path) = prepaint.selection_path.take() {
                let color = if focused {
                    ...
                    cx.theme().selection.opacity(0.4)
                };
        ...
        if focused {
            ...
            // Draw a stable, semi-transparent caret for premium IDE feel when unfocused
            if let Some(cursor_bounds) = prepaint.cursor_bounds_with_scroll() {
                window.paint_quad(fill(cursor_bounds, cx.theme().caret.opacity(0.4)));
            }
        }
