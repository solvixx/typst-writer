...
pub trait WindowExt: Sized {
    ...
    fn handle_any(&self) -> gpui::AnyWindowHandle;
}
...
impl Root {
    ...
    fn render_sheet_layer_direct(
        &self,
        root_entity: Entity<Self>,
        window: &mut Window,
        cx: &mut App,
    ) -> Option<AnyElement> {
...
impl Render for Root {
    fn render(&mut self, window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        let root_entity = window
            .root::<Root>()
            .flatten()
            .expect("BUG: window first layer should be a gpui_component::Root.");

        let sheet_layer = self.render_sheet_layer_direct(root_entity, window, cx);
        let dialog_layer = self.render_dialog_layer_direct(window, cx);
        let notification_layer = self.render_notification_layer_direct();
