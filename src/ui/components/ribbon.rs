use gpui::*;
use gpui_component::{h_flex, v_flex, button::{Button, ButtonVariants}, ActiveTheme, Selectable, IconName};
...
impl Ribbon {
    ...
    fn render_tab_selector(&self, tab: RibbonTab, label: &'static str, icon: IconName, cx: &mut Context<Self>) -> impl IntoElement {
    ...
    fn render_action_button(&self, label: &'static str, action: &'static str, icon: Option<IconName>) -> impl IntoElement {
        ...
        btn.on_click(move |_, window, cx| {
                if let Some(ws) = workspace.upgrade() {
                    ws.update(cx, |this, cx| {
                        ...
                        let selection = this.selection.filter(|s| !s.is_empty() && s.start <= current_text.len() && s.end <= current_text.len());
                            ...
                            "bold" | "italic" => {
                                ...
                                let process_format = |sel_start: usize, sel_end: usize| {
                                    let clamp_to_char_boundary = |pos: usize, text: &str| -> usize {
                                     ...
                                     let start = clamp_to_char_boundary(sel_start.min(sel_end).min(current_text.len()), &current_text);
                                     let end = clamp_to_char_boundary(sel_start.max(sel_end).min(current_text.len()), &current_text);
                                    ...
                                    if has_shorthand_inside {
                                        ...
                                        let replacement = current_text[start + func_open.len()..end - 1].to_string();
                                    ...
                                    let left_char = if start > 0 { current_text[..start].chars().next_back() } else { None };
                                    let right_char = if end < current_text.len() { current_text[end..].chars().next() } else { None };
                                    
                                    let left_is_alphanumeric = left_char.map_or(false, |c| c.is_alphanumeric());
                                    let right_is_alphanumeric = right_char.map_or(false, |c| c.is_alphanumeric());
                                    ...
                                    if left_is_alphanumeric || right_is_alphanumeric {
                                        ...
                                        let replacement = format!("{}{}{}", symbol, selected_text, symbol);
                                        let new_len = replacement.len();
                                ...
                                if let Some(sel) = selection {
                                    ...
                                    while word_start > 0 {
                                        let prev_char = current_text[..word_start].chars().next_back().unwrap();
                                    ...
                                    while word_end < current_text.len() {
                                        let next_char = current_text[word_end..].chars().next().unwrap();
                                    ...
                                    if word_start < word_end {
                                        ...
                                        let placeholder = if is_bold { " *bold text* " } else { " _italic text_ " };
                                        let cur_rel = if is_bold { 11 } else { 13 };
                                        let start_offset = 2;
                                        let len_offset = if is_bold { 9 } else { 11 };
                                        let new_sel = (edit_pos + start_offset)..(edit_pos + start_offset + len_offset);
    ...
    fn render_group(&self, name: &'static str, items: Vec<AnyElement>, cx: &mut Context<Self>) -> impl IntoElement {
        v_flex()
            .gap_1()
            .items_center()
            .justify_between()
            .child(
                h_flex().gap_1().children(items)
            )
            .child(
                div()
                    .text_xs()
...
impl Render for Ribbon {
    fn render(&mut self, _window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        let is_compiling = self.workspace.upgrade().map(|ws| ws.read(cx).compiler.is_compiling).unwrap_or(false);
            ...
            right_actions = right_actions.child(
                Button::new("toggle-preview")
                    ...
                    .on_click({
                        ...
                        move |_, window, cx| {
                            ws_handle.update(cx, |this, cx| {
                                this.config.preview_panel_visible = !this.config.preview_panel_visible;
                                crate::core::config::ConfigManager::save(&this.config);
                                this.update_panel_visibility(window, cx);
                                cx.notify();
                            });
        ...
        let content = match self.active_tab {
            RibbonTab::Home => {
                h_flex()
                    .gap_4()
                    .child(self.render_group("Font", vec![
                        self.render_action_button("Bold", "bold", Some(IconName::Bold)).into_any_element(),
                        self.render_action_button("Italic", "italic", Some(IconName::Italic)).into_any_element(),
                    ], cx))
                    .child(div().w_px().h_full().bg(cx.theme().border))
                    .child(self.render_group("Paragraph", vec![
                        self.render_action_button("Heading", "heading", Some(IconName::Heading)).into_any_element(),
                        self.render_action_button("List", "list", Some(IconName::List)).into_any_element(),
                    ], cx))
                    .into_any_element()
            }
            RibbonTab::Insert => {
                h_flex()
                    .gap_4()
                    .child(self.render_group("Symbols", vec![
                        self.render_action_button("Formula", "math_formula", Some(IconName::Function)).into_any_element(),
                    ], cx))
                    .into_any_element()
            }
            RibbonTab::Math => {
                h_flex()
                    .gap_4()
                    .child(self.render_group("Structures", vec![
                        self.render_action_button("Fraction", "math_frac", Some(IconName::Fraction)).into_any_element(),
                        self.render_action_button("Power", "math_super", Some(IconName::Superscript)).into_any_element(),
                        self.render_action_button("Subscript", "math_sub", Some(IconName::Subscript)).into_any_element(),
                        self.render_action_button("Root", "math_root", Some(IconName::Sqrt)).into_any_element(),
                        self.render_action_button("Matrix", "math_mat", Some(IconName::Matrix)).into_any_element(),
                    ], cx))
                    .child(div().w_px().h_full().bg(cx.theme().border))
                    .child(self.render_group("Calculus", vec![
                        self.render_action_button("Sum", "math_sum", Some(IconName::Sigma)).into_any_element(),
                        self.render_action_button("Integral", "math_int", Some(IconName::Integral)).into_any_element(),
                    ], cx))
                    .child(div().w_px().h_full().bg(cx.theme().border))
                    .child(self.render_group("Symbols", vec![
                        self.render_action_button("α", "math_alpha", Some(IconName::Alpha)).into_any_element(),
                        self.render_action_button("β", "math_beta", Some(IconName::Beta)).into_any_element(),
                        self.render_action_button("γ", "math_gamma", Some(IconName::Gamma)).into_any_element(),
                        self.render_action_button("θ", "math_theta", Some(IconName::Theta)).into_any_element(),
                        self.render_action_button("ω", "math_omega", Some(IconName::Omega)).into_any_element(),
                        self.render_action_button("π", "math_pi", Some(IconName::Pi)).into_any_element(),
                    ], cx))
                    .into_any_element()
            }
            RibbonTab::Developer => {
                h_flex()
                    .gap_4()
                    .child(self.render_group("Bindings", vec![
                        self.render_action_button("Let", "code_let", Some(IconName::Variable)).into_any_element(),
                    ], cx))
                    .child(div().w_px().h_full().bg(cx.theme().border))
                    .child(self.render_group("Rules", vec![
                        self.render_action_button("Set Rule", "code_set", Some(IconName::Settings2)).into_any_element(),
                        self.render_action_button("Show Rule", "code_show", Some(IconName::Eye)).into_any_element(),
                    ], cx))
                    .child(div().w_px().h_full().bg(cx.theme().border))
                    .child(self.render_group("Config", vec![
