...
pub fn paint_frame(
    ...
    font_cache: &std::sync::Arc<std::sync::Mutex<std::collections::HashMap<u64, FontId>>>,
) {
    let mut locked_cache = font_cache.lock().unwrap();
    paint_frame_rec(
        frame,
        origin,
        origin.y,
        ...
        match item {
            FrameItem::Text(text_item) => {
                ...
                for glyph in &text_item.glyphs {
                    ...
                    if Some(span) != last_span {
                        ...
                        if !span.is_detached() {
                            if let Some(node) = source.find(span) {
                                ...
                                let is_placeholder = is_math_placeholder(&node);
                    ...
                    if let Some((offset, is_ph, is_rad)) = last_span_data {
