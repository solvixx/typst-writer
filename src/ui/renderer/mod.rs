...
use gpui_component::{ActiveTheme, IconName, RopeExt, Sizable, h_flex, v_flex};
...
impl gpui::EntityInputHandler for RendererView {
    ...
    fn marked_text_range(
        ...
        self.composition_offset
            .map(|offset| offset..offset + self.composition_length)
    }

    fn unmark_text(&mut self, window: &mut Window, cx: &mut Context<Self>) {
        if let Some(ws_handle) = self
            .composition_offset
            .filter(|_| self.composition_length > 0)
            .and_then(|_| self.workspace.upgrade())
        {
            let offset = self.composition_offset.unwrap();
            ws_handle.update(cx, |ws, cx| {
                ws.apply_virtual_editor_action(
    ...
    fn bounds_for_range(
        ...
        if let Some(ws_handle) = self.workspace.upgrade() {
            let ws = ws_handle.read(cx);
            if let Some(((caret_idx, rel_cursor, cursor_h_px), doc)) =
                ws.cursor_relative_pos.zip(self.compiled_document.as_ref())
            {
...
impl RendererView {
    pub fn new(workspace: WeakEntity<EditorWorkspace>, cx: &mut Context<Self>) -> Self {
        ...
        cx.bind_keys([
            ...
            KeyBinding::new(
                "pageup",
                gpui_component::input::MovePageUp,
                Some("Renderer"),
            ),
            KeyBinding::new(
                "pagedown",
                gpui_component::input::MovePageDown,
                Some("Renderer"),
            ),
        ]);
        ...
        cx.subscribe(
            &this.zoom_slider_state,
            |this, _state, event: &gpui_component::slider::SliderEvent, cx| match event {
    ...
    fn handle_mouse_move(
        ...
        ws_handle.update(cx, |this_ws, cx| {
            if !this_ws.is_dragging {
                return;
            }
            if let Some(doc) = &this_ws.compiled_document {
                if let Some(page) = doc.pages.get(page_idx) {
                    let mut boxes = Vec::new();
                    geometry::collect_glyph_boxes_with_source(
                        &page.frame,
                        Point::default(),
                        this_ws.world.source_ref(),
                        &mut boxes,
                        zoom,
                    );
                    if let Some((raw_offset, _, _)) =
                        geometry::find_closest_offset(&boxes, rel_click, this_ws.world.source_ref())
                    {
                        ...
                        let editor_len = if let Some(active_path) = &this_ws.active_editor_path {
                            this_ws
                                .editors
                                .get(active_path)
                                .map(|e| e.read(cx).input.read(cx).text().len())
                        ...
                        if let Some(ref mut sel) = this_ws.selection {
                            ...
                            this_ws.selection =
                                Some(gpui_component::input::Selection::new(offset, offset));
                        }
                        ...
                        if let Some(view) = active_editor {
                            ...
                            if let Some(prev) = previous_focus
                                .filter(|p| *p != editor_focus && editor_focus.is_focused(window))
                            {
                                prev.focus(window);
                            }
                        }
    ...
    fn handle_key_down(&mut self, ev: &KeyDownEvent, window: &mut Window, cx: &mut Context<Self>) {
        ...
        if is_nav
            && let Some((page_idx, pos, _height)) = self
                .compiled_document
                .as_ref()
                .and_then(|_| ws_handle.read(cx).cursor_relative_pos)
        {
            ...
            if let Some(doc) = self.compiled_document.as_ref() {
                if let Some(page) = doc.pages.get(target_page) {
                    ...
                    geometry::collect_glyph_boxes_with_source(
                        &page.frame,
                        Point::default(),
                        ws_handle.read(cx).world.source_ref(),
                        &mut boxes,
                        self.zoom,
                    );
                    ...
                    if let Some((new_offset, _new_pos, _new_h)) = geometry::move_in_direction(
                        &boxes,
                        zoomed_pos,
                        key,
                        current_offset,
                        ...
                        ws_handle.update(cx, |ws, cx| {
                            if ev.keystroke.modifiers.shift {
                                ...
                                let range = if new_offset < anchor {
                                    new_offset..anchor
                                } else {
                                    anchor..new_offset
                                };
                                ws.apply_editor_action(
                                    crate::core::editor::EditorAction::Select {
                                        range,
                                ...
                                ws.apply_editor_action(
                                    crate::core::editor::EditorAction::MoveCursor {
            ...
            if !found && (key == "up" || key == "down") {
                let next_page = if key == "up" {
                    target_page.checked_sub(1)
                } else {
                    Some(target_page + 1)
                };
                if let Some(p) = next_page {
                    if let Some(doc) = self.compiled_document.as_ref() {
                        if let Some(page) = doc.pages.get(p) {
                            ...
                            let start_y = if key == "up" {
                                px(page.frame.height().to_pt() as f32 * PT_TO_PX * self.zoom)
                            } else {
                                px(0.0)
                            };
                            ...
                            if let Some((new_offset, _, _)) = geometry::move_in_direction(
                                &boxes,
                                start_pos,
                                key,
                                current_offset,
    ...
    pub fn update_document(
        &mut self,
        doc: std::sync::Arc<PagedDocument>,
        version: usize,
        cx: &mut Context<Self>,
    ) {
    ...
    fn render_typst_document(
        ...
        let source_paint = workspace
            .compiled_source
            .clone()
            .unwrap_or_else(|| workspace.world.source_ref().clone());
        ...
        let total_height = if num_pages > 0 {
            ...
            let last_page_top = self
                .page_tops
                .get(last_page_idx)
                .cloned()
                .unwrap_or(px(0.0));
            let last_page_height = doc
                .pages
                .get(last_page_idx)
        ...
        div().relative().w_full().h(total_height).children(
                    ...
                    div()
                        ...
                        .child(
                            div()
                                ...
                                .child(
                                    canvas(
                                        ...
                                        move |bounds, _prepaint, window, cx| {
                                            ...
                                            {
                                                let mut origins =
                                                    page_canvas_origins.lock().unwrap();
                                                if origins.len() <= i {
                                            ...
                                            if let Some(page) = doc_for_canvas.pages.get(i) {
                                                ...
                                                if let Some((_caret_idx, rel_cursor, cursor_h_px)) =
                                                    absolute_cursor.filter(|(idx, _, _)| {
                                                        *idx == i && caret_visible
                                                    })
                                                {
                                                    let cursor_h = px(cursor_h_px * zoom * 0.9);
                                                    let caret_top =
                                                        rel_cursor.y * zoom - cursor_h * 0.82;
                                                    let cursor_bounds = Bounds {
                                                        origin: point(
                                                            bounds.origin.x + rel_cursor.x * zoom,
                                                            bounds.origin.y + caret_top,
                                                        ),
                                ...
                                .on_mouse_up(
                                    MouseButton::Left,
                                    cx.listener(move |this, _, _, cx| this.handle_mouse_up(cx)),
                                ),
                        ),
                )
            }),
    ...
    fn render(&mut self, _window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        let inner_view = div()
            ...
            .child(
                canvas(
                    ...
                    move |bounds, _, window, cx| {
                        let handler =
                            gpui::ElementInputHandler::new(bounds, view_entity_for_canvas.clone());
            ...
            .on_action(cx.listener(
                |this, _action: &crate::ui::workspace::OpenCommandPalette, window, cx| {
                    this.focus_handle(cx).focus(window);
                },
            ))
            .on_action(
                cx.listener(|this, _action: &gpui_component::input::Copy, _window, cx| {
                    let ws_handle = this.workspace.upgrade().expect("Workspace dropped");
                    ws_handle.update(cx, |ws, cx| {
            ...
            .on_action(
                cx.listener(|this, _action: &gpui_component::input::Cut, window, cx| {
                    let ws_handle = this.workspace.upgrade().expect("Workspace dropped");
                    ws_handle.update(cx, |ws, cx| {
                        if let Some(sel) = &ws.selection {
                            // Normalize: selection may be stored backwards (start > end)
            ...
            .on_action(
                cx.listener(|this, _action: &gpui_component::input::Paste, window, cx| {
                    let ws_handle = this.workspace.upgrade().expect("Workspace dropped");
            ...
            .on_action(
                cx.listener(|this, _action: &gpui_component::input::Undo, window, cx| {
                    let ws_handle = this.workspace.upgrade().expect("Workspace dropped");
                    ws_handle.update(cx, |ws, cx| {
                        ws.undo(window, cx);
                    });
            ...
            .on_action(
                cx.listener(|this, _action: &gpui_component::input::Redo, window, cx| {
                    let ws_handle = this.workspace.upgrade().expect("Workspace dropped");
                    ws_handle.update(cx, |ws, cx| {
                        ws.redo(window, cx);
                    });
            ...
            .on_action(cx.listener(
                |this, _action: &gpui_component::input::MovePageUp, _window, cx| {
            ...
            .on_action(cx.listener(
                |this, _action: &gpui_component::input::MovePageDown, _window, cx| {
                    let viewport_height = this.scroll_handle.bounds().size.height;
                    let mut offset = this.scroll_handle.offset();
                    offset.y -= viewport_height * 0.9;
                    this.scroll_handle.set_offset(offset);
            ...
            .child(
                div()
                    ...
                    .child(v_flex().min_w(content_width).min_h_full().p_6().child(
            ...
            .child(div().absolute().right_0().top_0().bottom_0().child(
                gpui_component::scroll::Scrollbar::vertical(&self.scroll_handle),
            ))
            .child(div().absolute().left_0().bottom_0().right_0().child(
                gpui_component::scroll::Scrollbar::horizontal(&self.scroll_handle),
            ));
        ...
        div()
            ...
            .context_menu({
                ...
                move |menu, _, cx| {
                    ...
                    .menu_with_enable("Paste", Box::new(gpui_component::input::Paste), has_paste)
                    .separator()
                                        ...
                                        typst_pdf::pdf(doc, &typst_pdf::PdfOptions::default())
                                    {
                                        if let Some(ws_handle) = workspace.upgrade() {
                                            ...
                                            ws_handle.update(cx, |ws, _cx| {
                                                ws.log(
                                                    "INFO",
                                                    format!("Exported PDF to {:?}", pdf_path),
                                                );
                                            });
                                        }
                                    }
...
impl gpui_component::dock::Panel for RendererView {
            ...
            h_flex()
                ...
                .child(
                    Button::new("zoom-out")
                        ...
                        .on_click(cx.listener(|this, _, window, cx| {
                            ...
                            this.zoom = new_zoom;
                            this.zoom_slider_state
                                .update(cx, |state, cx| state.set_value(new_zoom, window, cx));
                            this.recalculate_layout();
                            cx.notify();
                        })),
                )
                ...
                .child(
                    div()
                        ...
                        .flex()
                        .justify_end()
                        .child(div().text_xs().child(format!("{}%", (zoom * 100.0) as i32))),
                )
                .child(
                    Button::new("reset-zoom")
                        ...
                        .on_click(cx.listener(|this, _, window, cx| {
                            this.zoom = 1.0;
                            this.zoom_slider_state
                                .update(cx, |state, cx| state.set_value(1.0, window, cx));
                            this.recalculate_layout();
                            cx.notify();
                        })),
                ),
    ...
    fn visible(&self, cx: &App) -> bool {
        self.workspace
            .upgrade()
            .map(|ws| ws.read(cx).config.preview_panel_visible)
            .unwrap_or(true)
    }
}
