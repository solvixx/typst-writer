...
pub struct EditorWorkspace {
    ...
    pub(crate) editors:
        std::collections::HashMap<std::path::PathBuf, Entity<crate::ui::editor::SourceEditorView>>,
...
impl EditorWorkspace {
    pub fn new(window: &mut Window, cx: &mut Context<Self>, _initial_text: &str) -> Self {
        ...
        // Components
        let title_bar = cx
            .new(|cx| crate::ui::components::title_bar::TitleBar::new(workspace_weak.clone(), cx));
        let ribbon =
            cx.new(|_cx| crate::ui::components::ribbon::Ribbon::new(workspace_weak.clone()));

        let editors = std::collections::HashMap::new();

        let renderer =
            cx.new(|cx| crate::ui::renderer::RendererView::new(workspace_weak.clone(), cx));

        let file_tree_entity = cx.new(|cx| {
            crate::ui::components::file_tree::FileTree::new(
        ...
        let dock_area = {
            ...
            cx.new(move |cx| {
                ...
                let mut dock_area =
                    DockArea::new("main-dock", None, window, cx).panel_style(PanelStyle::TabBar);

                let center_tabs =
                    DockItem::tab(renderer_for_dock, &cx.entity().downgrade(), window, cx);
                let center =
                    DockItem::h_split(vec![center_tabs], &cx.entity().downgrade(), window, cx);

                dock_area.set_center(center, window, cx);

                dock_area.set_left_dock(
                    DockItem::tabs(
                        vec![Arc::new(file_tree_for_dock)],
                        &cx.entity().downgrade(),
                        window,
                        cx,
                    ),
                ...
                // Bottom Dock: Logs
                let log_panel = cx.new(|_cx| {
                    crate::ui::components::log_panel::LogPanel::new(workspace_weak.clone(), _cx)
                });
    ...
    pub fn compile(&mut self, cx: &mut Context<Self>) {
        ...
        cx.spawn(move |_, cx: &mut AsyncApp| {
            ...
            async move {
                ...
                let result = cx
                    .background_executor()
                    .spawn(async move { typst::compile::<PagedDocument>(&world_clone) })
                    .await;
    ...
    fn sync_diagnostics(&mut self, diags: &EcoVec<SourceDiagnostic>, cx: &mut Context<Self>) {
        ...
        let mut file_diags: std::collections::HashMap<
            FileId,
            Vec<gpui_component::highlighter::Diagnostic>,
        > = std::collections::HashMap::new();

        for diag in diags {
            let id = if let Some(id) = diag.span.id() {
                id
            } else {
                continue;
            };
            let source = if let Ok(s) = self.world.source(id) {
                s
            } else {
                continue;
            };
    ...
    pub fn set_main_file(
        ...
        if let Ok(content) = std::fs::read_to_string(&path) {
            ...
            let vpath =
                typst::syntax::VirtualPath::within_root(&path, &root).unwrap_or_else(|| {
                    let filename = path
                        .file_name()
    ...
    pub fn change_working_directory(
        ...
        if !new_path.exists() || !new_path.is_dir() {
            self.log(
                "ERROR",
                format!("Cannot change to invalid directory: {:?}", new_path),
            );
            return;
        }
        ...
        self.log(
            "INFO",
            format!("Changed working directory to {:?}", new_path),
        );
            ...
            if let Ok(entries) = std::fs::read_dir(&new_path) {
                ...
                if let Some(typ_file) = first_typ {
                    ...
                    if let Err(e) = std::fs::write(&new_main, "= New Document\n") {
                        self.log(
                            "ERROR",
                            format!("Failed to create fallback main.typ: {}", e),
                        );
                    } else {
                ...
                self.log(
                    "ERROR",
                    format!("Failed to read directory entries for: {:?}", new_path),
                );
            }
        }
        cx.notify();
    ...
    pub fn new_document(&mut self, window: &mut Window, cx: &mut Context<Self>) {
        ...
        if std::fs::write(&path, &text).is_ok() {
            ...
            self.log(
                "ERROR",
                format!("Failed to create new document: {:?}", path),
            );
        }
    }
    ...
    pub fn open_preferences(&mut self, window: &mut Window, cx: &mut Context<Self>) {
        let workspace_weak = cx.weak_entity();
        let pref_view = cx.new(|cx| {
            crate::ui::components::preference_panel::PreferencePanel::new(workspace_weak, cx)
        });
        self.dock_area.update(cx, |dock, cx| {
            dock.add_panel(
    ...
    pub fn open_file(&mut self, path_str: &str, window: &mut Window, cx: &mut Context<Self>) {
        ...
        if let Ok(content) = std::fs::read_to_string(&path) {
            ...
            let editor_view = cx.new(|cx| {
                crate::ui::editor::SourceEditorView::new(
                    workspace_weak,
                    lsp_client,
                    window_handle,
                    window,
    ...
    pub fn apply_editor_action_for_file(
        &mut self,
        uri: &Url,
    ...
    pub fn apply_editor_action_from_editor(
        &mut self,
        uri: &Url,
        action: crate::core::editor::EditorAction,
        window: &mut Window,
    ...
    pub fn apply_editor_action(
        ...
        let active_uri = self
            .active_editor_path
            .as_ref()
            .and_then(|p| Url::from_str(&format!("file://{}", p.to_string_lossy())).ok());
        if let Some(uri) = active_uri {
            self.apply_editor_action_internal_for_file_opt(&uri, action, true, true, window, cx);
        }
    }

    pub fn apply_virtual_editor_action(
        &mut self,
        action: crate::core::editor::EditorAction,
        window: &mut Window,
        cx: &mut Context<Self>,
    ...
    fn apply_editor_action_internal(
        &mut self,
        action: crate::core::editor::EditorAction,
        record_undo: bool,
        window: &mut Window,
        ...
        if let Some(uri) = main_uri {
            self.apply_editor_action_internal_for_file_opt(
                &uri,
                action,
                record_undo,
                true,
    ...
    fn apply_editor_action_internal_for_file_opt(
        ...
        let path = uri
            .to_file_path()
            .unwrap_or_else(|_| std::path::PathBuf::from(uri.path().to_string()));
        ...
        match action {
            EditorAction::Edit {
                ...
                let old_selection = if is_active {
                ...
                if is_active {
                    if let Some(sel) = new_selection {
                        self.selection =
                            Some(gpui_component::input::Selection::new(sel.start, sel.end));
                    } else {
                        self.selection = None;
                    }
                }
            ...
            EditorAction::Select { range, reversed } => {
                if is_active {
                    self.selection = Some(gpui_component::input::Selection::new(
                        range.start,
                        range.end,
                    ));
                ...
                view.update(cx, |view, cx| {
                    view.input.update(cx, |state, cx| {
                        let target_sel = self.selection.unwrap_or_else(|| {
                            gpui_component::input::Selection::new(
                                self.cursor_offset,
                                self.cursor_offset,
                            )
                        });
                        let reversed = self.cursor_offset == target_sel.start
                            && target_sel.start != target_sel.end;
                        if state.selected_range() != target_sel
    ...
    pub fn update_cursor_node_info(&mut self, _cx: &mut Context<Self>) {
        ...
        if let Some(node) = root.leaf_at(
            self.cursor_offset.min(source.text().len()),
            ...
            self.ribbon
                .update(_cx, |ribbon, cx| ribbon.set_tab(tab, cx));
        }
    }
    ...
    pub fn refresh_caret_location(&mut self, cx: &mut Context<Self>, scroll_to_caret: bool) {
        if let Some(doc) = &self.compiled_document {
            ...
            for (idx, page) in doc.pages.iter().enumerate() {
                if let Some((pos, height, closest_dist)) = geometry::find_cursor_position(
                    &page.frame,
                    Point::default(),
                    target_offset,
                    compiled_source,
