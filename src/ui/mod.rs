...
fn read_postscript_name(face: &ttf_parser::Face<'_>) -> String {
    ...
    for record in face.names() {
        match record.name_id {
            1  => { if let Some(s) = record.to_string() { family = s; } }
            2  => { if let Some(s) = record.to_string() { subfamily = s; } }
            6  => { if let Some(s) = record.to_string() { ps_name = s; } }
            _  => {}
        }
    }
    if !ps_name.is_empty() { return ps_name; }
    if subfamily.is_empty() { family } else { format!("{}-{}", family, subfamily) }
}
...
pub fn patch_font_family_safe(data: &mut [u8], font_id: u32, face_idx: u32) {
    ...
    if data.starts_with(b"ttcf") {
        ...
        if face_idx < num_fonts {
            ...
            face_offset = u32::from_be_bytes([data[offset_pos], data[offset_pos+1], data[offset_pos+2], data[offset_pos+3]]) as usize;
            
            if face_idx != 0 {
                let offset0_pos = 12;
                let offset0 = u32::from_be_bytes([data[offset0_pos], data[offset0_pos+1], data[offset0_pos+2], data[offset0_pos+3]]);
    ...
    fn find_table(data: &[u8], face_offset: usize, tag: &[u8; 4]) -> Option<(usize, usize, usize)> {
        if data.len() < face_offset + 12 { return None; }
        let num_tables = u16::from_be_bytes([data[face_offset + 4], data[face_offset + 5]]) as usize;
    ...
    if let Some((record_offset, name_offset, table_len)) = find_table(data, face_offset, b"name") {
        if data.len() < name_offset + 6 { return; }
        let num_records = u16::from_be_bytes([data[name_offset + 2], data[name_offset + 3]]) as usize;
        let storage_offset = u16::from_be_bytes([data[name_offset + 4], data[name_offset + 5]]) as usize;
        
        for i in 0..num_records {
            let rec_offset = name_offset + 6 + i * 12;
            if data.len() < rec_offset + 12 { break; }
            ...
            if [1, 4, 16, 21].contains(&name_id) {
                let length = u16::from_be_bytes([data[rec_offset + 8], data[rec_offset + 9]]) as usize;
                let string_offset = u16::from_be_bytes([data[rec_offset + 10], data[rec_offset + 11]]) as usize;
                let absolute_offset = name_offset + storage_offset + string_offset;
                
                if data.len() < absolute_offset + length { continue; }

                let id_str = format!("TF{:04}", font_id);

                if platform_id == 3 || platform_id == 0 { // Windows / Unicode (UTF-16BE)
                    let chars_count = length / 2;
                    if chars_count >= id_str.len() {
                        let mut replacement = id_str.clone();
                        while replacement.len() < chars_count { replacement.push('X'); }
                ...
                } else if platform_id == 1 { // Mac Roman
                    if length >= id_str.len() {
                        let mut replacement = id_str.clone();
                        while replacement.len() < length { replacement.push('X'); }
