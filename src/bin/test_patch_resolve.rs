...
fn find_table(data: &[u8], tag: &[u8; 4]) -> Option<usize> {
    ...
    for i in 0..num_tables {
        let offset = 12 + i * 16;
        if data.len() < offset + 16 { break; }
        if &data[offset..offset+4] == tag {
            return Some(u32::from_be_bytes([data[offset+8], data[offset+9], data[offset+10], data[offset+11]]) as usize);
        }
    }
    None
}
            ...
            if [1, 4, 16, 21].contains(&name_id) {
                ...
                if platform_id == 3 {
                    ...
                    if length >= utf16_name.len() * 2 {
                        ...
                        for j in (utf16_name.len() * 2)..length { data[absolute_offset + j] = 0; }
                    }
                } else if platform_id == 1 {
                    let bytes = new_name.as_bytes();
                    if length >= bytes.len() {
                        for (j, &b) in bytes.iter().enumerate() { data[absolute_offset + j] = b; }
                        for j in bytes.len()..length { data[absolute_offset + j] = 0; }
                    }
                }
            }
        }
...
fn main() {
    Application::new().run(|cx| {
        ...
        cx.text_system().add_fonts(vec![std::borrow::Cow::Owned(data)]).unwrap();
